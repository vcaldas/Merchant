...
cursor.execute(
    """SELECT symbol ,name, id, market FROM stock WHERE market = 'CRYPTO'""")
   ...
   # no keys required for crypto data
    ...
    for _symbol in symbol_chunck:
        ...
        for entry in bar:
            ...
            VALUES (?, ?,?,?,?,?,?, ?)
        """, (stock_id, entry.timestamp.date().isoformat(), entry.open, entry.high, entry.low, entry.close, entry.volume, entry.vwap))
