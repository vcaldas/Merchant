...
for asset in assets:
    if (
        asset.status.name.upper() == "ACTIVE"
        and asset.tradable
        and asset.symbol not in symbols
    ):
        try:
            cursor.execute(
                "INSERT INTO stock (symbol, name, market, exchange) VALUES (? ,?, ?, ?)",
                (asset.symbol, asset.name, asset.asset_class.name, asset.exchange.name),
            )
